...
def TRAIN(train):
    ...
    while distance[-1] > 20 :
        ...
        if (anomtype[30][count]== 0)&(n%2==0):
                
            print("here")
            ULTRA()
            count += 1
        elif (anomtype[30][count]== 2)&(n%2==0):
                
            print("two")
            count += 1

        elif (anomtype[30][count] == 5)&(n%2==0) :
             
            if ((S1 == 1) and (S3 == 1) and (S2 == 0)):
                score.append(count)
            speedtime = speedfactor+anomduration[30][count]*0.03
            ...
            stop()
            print("completed")
            anomnonzero = list(filter(lambda num: num != 0, anom[n]))
            anomdurationnonzero = list(filter(lambda num: num != 0, anomduration[n]))
            for num in anomnonzero :
                match.append(anomduration[30][num]-anomduration[n][num])
                newcount = anomnonzero[match.index(min(match))]
            if (newcount - count) <10:
                count = newcount
            count +=1  

        elif (anomtype[30][count] == 6)&(n%2==0) :
            ...
            speedtime = speedfactor+(anomduration[30][count])*0.03
            speed1 = 35+(anomduration[30][count])*0.05
            speed2 = 0+(anomduration[30][count])*0.001
            frq = 11
        ...
        elif (anomtype[30][count] == 4)&(n%2==0) :
        ...
        elif ((S1 == 1) and (S3 == 1) and (S2 == 0))&(n%2!=0) :
    ...
    if n > 0:
        ...
        if user == 'y':
            ...
            print("model updated\n model = ",list(filter(lambda num: num != 0, anom[30])))
        else :
